...
const MessageItem = ({ _id, message, sender, createdAt, fontSize, isSeen }) => {
  const { socket } = useContext(ChatContext);
  const time = moment(createdAt).format("h:mm a");
  const { id } = useGetUserId();
  const handleChangeVisibility = (isVisibility) => {
    if (isVisibility === true && isSeen == false && sender._id !== id) {
      socket.emit("message:seen", { id: _id, userId: sender._id });
    }
  };
