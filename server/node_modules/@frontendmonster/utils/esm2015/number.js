 ...
 */
export const safeDivide = (dividend, divisor, fallback = 0) => isNumber(dividend / divisor) ? dividend / divisor : fallback;
/**
 ...
 */
export const clamp = (value, [min, max] = [0, 1]) => Math.max(Math.min(value, max), min);
/**
 ...
 */
export const percent = (value, max) => clamp(safeDivide(value, max)) * 100;
/**
 * random int number between two value
 * @param {range} [range = { min = 0, max = MAX_SAFE_INTEGER }] - range of random numbers
 * @returns {number} random integer between min and max
 */
export const randomInt = ({ min = 0, max = Number.MAX_SAFE_INTEGER } = {}) => Math.floor(Math.random() * (max - min)) + min;
/**
 * parse number to integer in radix 10
 * @param {string} num number to parse
 * @returns {number} parsed number
 */
export const toInteger = (num) => Number.parseInt(num, 10);
/**
